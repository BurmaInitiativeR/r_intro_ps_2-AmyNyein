---
title: "r_intro PS 2_answer_nyein"
author: "Nyein"
date: "2022-11-16"
output: pdf_document
---
...
# sample size=30 and replication time=200
```{r means_list_2}
set.seed(234567)
means_list<- replicate (n=200, expr=mean(sample(df$Sepal.Length, size=30, replace=TRUE)))
test<- rep(2,100)
cbind(test,means_list)
...
# sample size=50 and replication time=1000

```{r means_list_3}

set.seed(345678)
means_list<- replicate(n=1000, expr=mean(sample(df$Sepal.Length, size=50, replace=TRUE)))
test<- rep(3,1000)
cbind(test,means_list)
...
# sample size=50 and replication time=3000

```{r means_list_4}

set.seed(456789)
means_list<- replicate (n=3000, expr=mean(sample(df$Sepal.Length, size=50, replace=TRUE)))
test<- rep(4,1000)
cbind(test,means_list)
...
# sample size 50 and replication time 10,000
```{r means_list_5}

set.seed(567890)
means_list<- replicate (n=10000, expr=mean(sample(df$Sepal.Length, size=50, replace=TRUE)))
test<- rep(5,1000)
cbind(test,means_list)
...
```{r means_series_1to5}
means_series<- list(means_list_1, means_list_2, means_list_3, means_list_4, means_list_5)
means_series
```
...
```
means_seris[[1]]
```{r mean value of column mean_list}
...
```{r combine data set from means_series}
